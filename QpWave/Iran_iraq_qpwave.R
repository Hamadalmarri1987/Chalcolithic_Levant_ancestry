...
for(i in seq_along(combo_list)){
  ...
  # Extract f2 summary
  snp_total    <- attr(f2_blocks, "nsnps_read")
  snp_filtered <- attr(f2_blocks, "nsnps")
  snp_poly     <- attr(f2_blocks, "nsnps_poly")
  
  # Print them in console
  cat("✔", snp_total, "SNPs read in total\n")
  cat("!", snp_filtered, "SNPs remain after filtering.",
      snp_poly, "are polymorphic.\n")
  ...
  results[[name]] <- list(
    snps_total    = snp_total,
    snps_filtered = snp_filtered,
    snps_poly     = snp_poly,
